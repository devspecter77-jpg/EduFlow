...
import { teachersApi, type Teacher, type TeacherFilters } from '@/lib/api/teachers';
...
type SortField = 'fullName' | 'phone' | 'experience' | 'status';
...
export function Teachers() {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        Array.from(selectedIds).map(id => teachersApi.delete(id))
      );
      setBulkDeleteOpen(false);
      setSelectedIds(new Set());
      showToast('success', `${selectedIds.size} ta o'qituvchi o'chirildi`);
      load();
    } catch {
      showToast('error', 'O\'chirishda xatolik yuz berdi');
    } finally {
    ...
    try {
      await Promise.all(
        Array.from(selectedIds).map(id =>
          teachersApi.update(id, { status: bulkStatus as Teacher['status'] })
        )
      );
      setBulkStatusOpen(false);
      setBulkStatus('');
      setSelectedIds(new Set());
      showToast('success', `${selectedIds.size} ta o'qituvchi holati o'zgartirildi`);
      load();
    } catch {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground sm:text-3xl">O'qituvchilar</h1>
          <p className="mt-1 text-muted-foreground">
            Jami: <span className="font-medium text-foreground">{total}</span> ta o'qituvchi
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => importExportApi.exportTeachers(selectedIds.size > 0 ? Array.from(selectedIds) : undefined)}
            className="inline-flex items-center gap-2 rounded-lg border bg-background px-3 py-2 text-sm hover:bg-accent transition-colors"
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-muted/50">
                    ...
                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">Guruhlar</th>
                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">
                    ...
                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">
                      <button onClick={() => handleSort('experience')} className="flex items-center gap-1 hover:text-foreground transition-colors">
                        Tajriba
                        <SortIcon field="experience" sortField={sortField} sortOrder={sortOrder} />
                      </button>
                    </th>
                    <th className="px-4 py-3 text-left font-medium text-muted-foreground">
                ...
                <tbody>
                  {teachers.map((t, idx) => {
                    ...
                    const groupNames = t.groupIds.map(id => groupMap[id] || id).join(', ') || 'Guruh yo\'q';
                    ...
                    return (
                      <tr key={t.id} className={`border-b last:border-0 hover:bg-muted/30 transition-colors ${isSelected ? 'bg-teal-50/50 dark:bg-teal-900/10' : ''}`}>
                        ...
                        <td className="px-4 py-3 text-muted-foreground">{groupNames}</td>
                        <td className="px-4 py-3 text-muted-foreground">{t.phone}</td>
                        <td className="px-4 py-3 text-muted-foreground">{t.experience} yil</td>
                        <td className="px-4 py-3">
                          <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[t.status]}`}>
                            {STATUS_LABELS[t.status]}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          <BirthdayCell birthDate={t.birthDate} />
                        </td>
                        <td className="px-4 py-3">
      ...
      {editTeacher && (
        <TeacherModal
          ...
          onSuccess={() => { 
            setEditTeacher(null); 
            load(); 
            showToast('success', 'O\'qituvchi muvaffaqiyatli yangilandi'); 
          }}
        />
      )}
      {viewTeacher && (
      ...
      {deleteTeacher && (
        <ConfirmModal
          ...
          message={`"${deleteTeacher.fullName}" ni o'chirishni tasdiqlaysizmi? Bu amal bekor qilinmaydi.`}
      ...
      {bulkDeleteOpen && (
        <ConfirmModal
          ...
          message={`${selectedIds.size} ta o'qituvchini o'chirishni tasdiqlaysizmi? Bu amal bekor qilinmaydi.`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setBulkStatusOpen(false)}>
          <div className="w-full max-w-md rounded-2xl bg-card border shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-muted-foreground mb-4">
              {selectedIds.size} ta o'qituvchining holatini o'zgartirish
            </p>
            <select
              value={bulkStatus}
