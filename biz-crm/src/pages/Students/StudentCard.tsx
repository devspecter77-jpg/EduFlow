import { Eye, Pencil, Trash2, CheckSquare, Square, Phone, Calendar, Users, Cake } from 'lucide-react';
...
export function StudentCard({
  student, groupName, isSelected, onSelect, onView, onEdit, onDelete,
}: StudentCardProps) {
  const bdayInfo = student.birthDate ? getBdayInfo(student.birthDate) : null;
    ...
    <div className={`relative rounded-xl border bg-card p-4 hover:shadow-md transition-all ${urgencyBorder}`}>
      ...
      <div className={bdayInfo ? 'pt-2' : ''}>
        ...
        <div className="flex items-start justify-between mb-3">
          <button onClick={onSelect} className="flex-shrink-0 mt-0.5">
            {isSelected
              ? <CheckSquare className="h-5 w-5 text-teal-600" />
              : <Square      className="h-5 w-5 text-muted-foreground" />}
          </button>
          <div className="flex items-center gap-1">
            <button onClick={onView}   className="rounded-md p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground transition-colors" title="Ko'rish">
              <Eye    className="h-4 w-4" />
            </button>
            <button onClick={onEdit}   className="rounded-md p-1.5 text-muted-foreground hover:bg-accent hover:text-foreground transition-colors" title="Tahrirlash">
              <Pencil className="h-4 w-4" />
            </button>
            <button onClick={onDelete} className="rounded-md p-1.5 text-muted-foreground hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-900/20 transition-colors" title="O'chirish">
        ...
        <div className="space-y-2">
          <div>
            <h3 className="text-base font-semibold text-foreground leading-tight">{student.fullName}</h3>
          ...
          <div className="flex items-center justify-between pt-1">
            <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[student.status]}`}>
